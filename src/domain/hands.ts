...
import { currentSuitOf } from "./template";
...
export interface HandRank { category: HandCategory; categoryScore: number; label: string; tiebreaker: number[]; }

const CATEGORY_SCORES: Record<HandCategory, number> = { "high-card": 0, pair: 1, "two-pair": 2, "three-of-a-kind": 3, straight: 4, flush: 5, "full-house": 6, "four-of-a-kind": 7, "straight-flush": 8 };
const CATEGORY_LABELS: Record<HandCategory, string> = { "high-card": "高牌", pair: "一對", "two-pair": "兩對", "three-of-a-kind": "三條", straight: "順子", flush: "同花", "full-house": "葫蘆", "four-of-a-kind": "鐵支", "straight-flush": "同花順" };

function sortedValues(cards: Card[]): number[] { return cards.map((card) => rankValue(card.rank)).sort((a, b) => b - a); }
function groups(cards: Card[]): Array<{ value: number; count: number }> {
  const counts = new Map<number, number>();
  for (const card of cards) counts.set(rankValue(card.rank), (counts.get(rankValue(card.rank)) ?? 0) + 1);
  return [...counts.entries()].map(([value, count]) => ({ value, count })).sort((a, b) => b.count - a.count || b.value - a.value);
}
...
function rank(category: HandCategory, tiebreaker: number[]): HandRank { return { category, categoryScore: CATEGORY_SCORES[category], label: CATEGORY_LABELS[category], tiebreaker }; }

export function evaluateHand(cards: Card[]): HandRank {
  if (cards.length !== 3 && cards.length !== 5) throw new Error("十三支分墩只能評估 3 張或 5 張牌");
  ...
  const flush = cards.every((card) => currentSuitOf(card) === currentSuitOf(cards[0]));
  ...
  if (handGroups[0].count === 2 && handGroups[1].count === 2) return rank("two-pair", [Math.max(handGroups[0].value, handGroups[1].value), Math.min(handGroups[0].value, handGroups[1].value), handGroups[2].value]);
