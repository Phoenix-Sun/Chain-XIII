...
export default function RunSessionView({ partyCharacterIds = ["water-scout"], seed = "CHAIN-XIII-RUN-001", initialRun, initialGeneInventory = [], onNavigate, onRunUpdated, onRunSettled }: { partyCharacterIds?: string[]; seed?: string; initialRun?: RunState; initialGeneInventory?: RunState["geneInventory"]; onNavigate?: Navigate; onRunUpdated?: (run: RunState) => void; onRunSettled?: (run: RunState) => void }) {
  ...
  if (phase === "route") return <div className="run-session"><div className="run-progress"><span>遠征進行中</span><strong>{run.earnedCrystals} 水晶</strong></div><RunRouteView run={run} onNodeSelected={selectNode} onOpenWorkshop={() => setPhase("workshop")} /></div>;
  if (phase === "battle") return <div className="run-session"><div className="run-progress"><span>目前節點：{currentNode.type === "boss" ? "Boss" : currentNode.type === "elite" ? "強敵" : "戰鬥"}</span><strong>{run.earnedCrystals} 水晶</strong></div><BattleArenaView partyCharacterIds={run.partyCharacterIds} node={currentNode} battleSeed={`${run.seed}:${currentNode.id}`} equippedGenes={run.equippedGenes} relicIds={run.relicIds} onBattleComplete={handleBattleComplete} /></div>;
  if (phase === "exploration") return <div className="run-session"><div className="run-progress"><span>目前節點：事件</span><strong>{run.earnedCrystals} 水晶</strong></div><ExplorationView node={currentNode} seed={run.seed} onResolved={(result) => handleExplorationComplete(result.success)} /></div>;
