...
export default function RunRouteView({ partyCharacterIds = ["water-scout"], run, onNodeSelected, onNavigate, onOpenWorkshop }: RunRouteViewProps) {
  ...
  return <section className="route-card" aria-labelledby="route-title">
    ...
    <div className="route-preview" aria-label="下一站資訊"><strong>下一站資訊</strong>{reachableNodes.map((node) => { const monster = node.monsterId ? catalog.monsters.find((candidate) => candidate.id === node.monsterId) : undefined; const reward = rewardForNode(node); return <div className="route-preview-item" key={node.id}><span>{NODE_NAMES[node.type]}{monster ? `・${monster.name}` : ""}</span><small>{monster ? `可能掉落 ${monster.dropChainPoolIds.length} 種基因鏈` : reward.detail}</small></div>; })}</div>
    <div className="route-rows">{rows.map((row) => <div className="route-row" key={row}><small>第 {row + 1} 層</small><div className="route-nodes">{activeRun.map.nodes.filter((node) => node.row === row).map((node) => <button type="button" key={node.id} className={`route-node node-${node.type}${node.id === activeRun.currentNodeId ? " is-current" : ""}${reachableIds.has(node.id) ? " is-reachable" : ""}${completedIds.has(node.id) ? " is-completed" : ""}`} disabled={!reachableIds.has(node.id)} onClick={() => travel(node.id)} aria-label={NODE_NAMES[node.type]} aria-pressed={node.id === activeRun.currentNodeId}><strong>{NODE_LABELS[node.type]}</strong></button>)}</div></div>)}</div>
