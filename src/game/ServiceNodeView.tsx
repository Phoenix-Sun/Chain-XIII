...
export default function ServiceNodeView({ node, run, onResolved }: { node: RunMapNode; run: RunState; onResolved: (run: RunState) => void }) {
  ...
  return <section className={`service-node-card service-${node.type}`} aria-labelledby="service-node-title"><h1 id="service-node-title">{title}</h1><p>{intro}</p><div className="service-choice-list">{choices.map((choice) => { const copy = COPY[choice]; const cost = SERVICE_COSTS[choice] ?? 0; const livesFull = choice.includes("heal") && run.livesRemaining >= run.maxLives; const crystalsMissing = run.earnedCrystals < cost; const unavailable = crystalsMissing || livesFull; const actionLabel = livesFull ? "命數已滿" : crystalsMissing ? "水晶不足" : cost > 0 ? `${cost} 水晶` : copy.button; return <button type="button" className="service-choice" disabled={busy || unavailable} key={choice} onClick={() => choose(choice)}><span><b>{copy.title}</b><small>{copy.detail}</small></span><strong>{actionLabel}</strong></button>; })}</div>{error && <p className="reward-error" role="alert">{error}</p>}</section>;
}
