...
- 狀態：採用
- 節點分布：一般路徑可生成普通戰鬥、事件、遺物祭壇、商隊、營火與瞭望台；一般路徑不生成 Elite，第 1、2 章最後固定為 Elite，第 3 章最後固定為 Boss。
...
- Save：RunMap 使用 `chapterEndNodeIds`；save version 7 能將舊版 `chapterBossNodeIds` 轉為相容欄位，並保存遺物祭壇、祝福與下一場 Skull 詛咒。
- 原因：讓 Elite 與 Boss 的強度來自可理解的牌面結構，而不是只有模糊數值加成；即使 Boss 失敗，前兩章獲得的水晶仍能推動 Meta 成長。

## ADR-012：基因鏈改為固定配置與自由啟用，不再合成升階

- 狀態：採用
- 定位：基因鏈是已完成的花色配置卡，不是素材、配方或需要長期經營的合成樹。
- 掉落資料：每條鏈在掉落時固定作用墩位（頭／中／尾）、固定 3 格或 5 格長度、固定元素順序與初始啟用狀態。
- 玩家操作：玩家可自由點選每一格開關，沒有啟動格數上限、啟動成本或 tier 升階；同一墩若有多條鏈，顯示為替換候選，同時只套用玩家選中的一條。
- 花色語意：未啟用格代表不覆寫該張牌，保留原始牌面花色；它不是一個可被克制的「無元素」。
- 強度：允許基因鏈成為很強的 Build。平衡來自固定墩位、元素排列、是否覆寫原牌與掉落稀有度，不用人為限制玩家可啟用的格數。
- UI：工房改為「基因鏈配置」，先依墩位分組，再以候選卡切換與大型格子按鈕操作；畫面必須同時顯示固定配置、目前啟用結果與 13 張牌花色預覽。
- 相容性：save version 6 將舊 tier 因子轉為無 tier 的固定配置，舊鏈預設所有格啟用；不再生成新的融合鏈 ID。
- 原因：保留尋找強力花色 Build 的樂趣，移除玩家不想投入時間的素材整理、不可逆融合與升階負擔。
