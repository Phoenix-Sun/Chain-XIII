...
- ✅ 難度、最大命數與剩餘命寫入 `RunState`，save version 4 支援舊 active Run migration 與 resume。
- ✅ 隊伍編成畫面提供難度選擇、命數說明；遠征 HUD 與結算畫面顯示剩餘命。
- ✅ 難度獎勵倍率明確分離：容易 0.75x、中等 1.00x、困難 1.50x；普通戰鬥、Elite、事件、遺物、Boss 與事件失敗保底均套用。

## 三章遠征節點

- ✅ RunMap 改為三章：第 1 章 10～13 節點、第 2 章 7～9 節點、第 3 章 4～6 節點；每章最後固定 1 個 Boss。
- ✅ 第一、第二章 Boss 領獎後回到路線，第三章 Boss 才進入最終結算。
- ✅ 後章提高 Elite 節點權重，並套用第 2 章敵方平手 +1、第 3 章 +2 的章節強度。
- ✅ save version 4 保存三章 Boss／長度 metadata，舊 active Run 可補上相容欄位。
