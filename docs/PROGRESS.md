...
| P2 | 固定基因配置、格子啟用、3/5/5 花色預覽 | ✅ Run 已接入第一 slice | 基因鏈依墩位分組；同墩多條鏈可切換，無融合與 tier 升階 |
| P3 | 可操作單場戰鬥、角色能力、敵 AI | 🟡 戰鬥完成、角色技能第一批接入 | 13 張批次分墩、敵 AI、三墩結果與節點回寫已接；水紋、風之預視、火花、四象、潮汐流轉、風行商人、石碑揭示與鍛造師全開基因格已有 Run 內操作入口；其餘角色 Effect 待接 |
| P4 | Seed 地圖、節點、普通怪、Elite、Boss 路線 | ✅ Run 閉環完成第一版 | 路線、戰鬥／事件／獎勵、Boss、結算可走通 |
|- P5 | Boss 規則、遺物祭壇、祝福、探索事件 | ✅ 第一版已接入 | Boss 模板與三種規則已進戰鬥；遺物祭壇固定五骰、Skull 鎖定／爆骰、收手與遺物二選一已接 Run；祝福會在下一場戰鬥生效；19 件遺物中 15 件直接影響戰鬥，其餘影響祭壇／路線／經濟；12 個事件有獨立目標與結果 |
| P6 | Meta、角色、升星、圖鑑與 MVP 內容 | 🟡 基本 Meta、升星、永久技能樹與圖鑑已接 | 水晶抽卡、角色收藏、重複轉印記、角色升星、3 個永久技能節點與遠征圖鑑均可用；更多 Skill Tree 節點與圖鑑深度仍待擴充 |
|- P7 | Save migration、錯誤紀錄、效能與 UX polish | 🟡 local-first 可續玩 | Save version 7 支援祭壇中間狀態、祝福與 Skull 詛咒 migration；仍待錯誤紀錄與手機實機驗證 |

## 已驗證能力

1. 所有抽牌與事件亂數集中到 `SeededRandom`；模板牌分離 `originalSuit` / `currentSuit`。
2. 元素環、牌型優先序、敵 AI 合法分牌、固定基因配置與 3/5/5 花色套用均由純函式測試覆蓋。
...
14. 玩家基因鏈現在真正進入 Run inventory；配置畫面依頭／中／尾墩分組，同一墩可切換多條固定配置並逐格啟用，模板會改變下一場牌的 currentSuit。
...
18. 19 件遺物各有可辨識的能力；其中 15 件直接影響三墩戰鬥，戰鬥桌會在確認牌局前顯示目前遺物效果。
...
- ✅ 鍛造師已改為基因配置 phase；啟用後會把目前三個墩位的基因格全部打開，不涉及素材或融合。
- ✅ 永久技能樹已接入隊伍整備：擴充基因袋、開局演練、路線網絡可消耗水晶解鎖；技能節點會持久化到 Meta，並在新 Run 實際改變基因容量、頭墩比較或起始水晶。
- ✅ 遠征圖鑑已接入主導覽：依 Meta 顯示已遭遇怪物、Boss 規則與永久遺物收藏；未知條目保持遮蔽，不把尚未取得的內容誤當成已解鎖。
- ✅ 12 個探索事件已由泛用佔位文字改為獨立的場景敘事與成功／失敗結果；路線預覽會先顯示事件名稱、目標與可能獎勵，擲骰結果會回報事件後果。
- ✅ 內容 validator 會拒絕缺少事件名稱、內容或成功／失敗結果文字的事件，避免新增內容只完成機制而沒有玩家可理解的目標回饋。
- ✅ 遺物祭壇已接入 Run：五顆固定六面骰、水晶／祝福成對結算、遺物三連二選一、Skull 鎖定與三顆爆骰；爆骰會清除未保護獎勵並施加下一場戰鬥詛咒。
- ✅ 祝福已接入下一場十三支戰鬥，戰鬥結束後自動消耗；遺物「無光護符／收手印／星圖碎片／戰利品印章」分別接入祭壇、爆骰保護、路線預覽與戰鬥經濟。
- ✅ 路線新增商隊、營火、瞭望台節點；商隊有補給／備戰／情報三選一，營火可休整或備戰，瞭望台可查看下一層。
...
- ✅ 普通戰敗扣 1 命、Elite 戰敗扣 2 命、Boss 戰敗直接清空剩餘命數；命未歸零時留在同一節點重新挑戰，命歸零才進入遠征結算。
- ✅ 難度、最大命數與剩餘命寫入 `RunState`，save version 7 支援舊 active Run migration、祭壇狀態與一次性戰鬥效果 resume。
- ✅ 隊伍編成畫面提供難度選擇、命數說明；遠征 HUD 與結算畫面顯示剩餘命。
- ✅ 難度獎勵倍率明確分離：容易 0.75x、中等 1.00x、困難 1.50x；普通戰鬥、Elite、事件、遺物、Boss 與事件失敗保底均套用。

## 三章遠征節點與候選敵方牌組

- ✅ RunMap 改為三章：第 1 章 10～13 節點、第 2 章 7～9 節點、第 3 章 4～6 節點；一般路徑沒有 Elite，第 1／2 章最後是 Elite，第 3 章最後是 Boss。
- ✅ Elite 從 2 組 deterministic 13 張候選牌中選較高階的合法 3／5／5 組合；Boss 從 3 組候選牌中選最強組合。
- ✅ 第一、第二章 Elite 領取水晶／基因鏈後回到路線，第三章 Boss 才進入最終結算；Boss 失敗是合理的 Run 結果，但前兩章獎勵仍能累積 Meta 成長。
- ✅ 章節平手微調改為第 1 章 +0、第 2 章 +0、第 3 章 +1；save version 5 保存 `chapterEndNodeIds` 並相容舊欄位。
